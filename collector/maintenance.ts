...
export function runMaintenance(db: Db, cfg: MaintenanceConfig, now: Date): void {
  ...
  if (cfg.rollupAfterDays > 0) {
    ...
    const sel = raw.prepare(
      `SELECT t.started_at, q.pair, q.amount_in, q.source_id, q.net_out, q.price_impact_pct, t.blnd_usd
    ...
    type Acc = { nets: bigint[]; impacts: number[]; blnd: number[]; dist: Record<string, number> };
    ...
    for (const r of rows) {
      ...
      if (!g) { g = { hour, pair, amount, acc: { nets: [], impacts: [], blnd: [], dist: {} } }; groups.set(key, g); }
    ...
    const upsert = raw.prepare(
      `INSERT INTO rollup_hourly (hour_utc, pair, amount_in, n_ticks, net_min, net_med, net_max, impact_avg, winner_dist, blnd_usd_avg)
       VALUES ($hour, $pair, $amount, $n, $min, $med, $max, $impact, $dist, $blnd)
    ...
    try {
      for (const g of groups.values()) {
        ...
        upsert.run({
          ...
          impact: avg(impacts), dist: JSON.stringify(g.acc.dist), blnd: avg(blnds),

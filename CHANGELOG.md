...
Chaque version publiée est reliée à sa Release GitHub, qui porte les notes auto-générées
(la liste complète des pull requests mergées). Ce fichier est le pendant curé, lisible par un
humain : il décrit ce qui a changé pour un utilisateur du projet, pas ce qui a été mergé.
...
- `npm ci` failed on `develop`: the lockfile was resolving `@stellar/stellar-sdk@16.1.0`,
  ...
  the lockfile only — no dependency range changed.

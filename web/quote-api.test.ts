// Tests for web/quote-api.ts — click-to-select properties of ladder rows.
// Mock the engine to avoid any network and any DB.
import { describe, it, expect, vi, beforeEach } from 'vitest';
import { BLND, USDC } from '../core/assets.js';

// ─── Mock engine ─────────────────────────────────────────────────────────────
// vi.mock must be at the top of the file (hoisted by vitest).
vi.mock('../core/engine.js', () => ({
  quote: vi.fn(),
}));
// Mock balance.js (imported by quote-api via walletBalance, not used here)
vi.mock('../core/balance.js', () => ({
  readBlndBalance: vi.fn(async () => 0n),
}));
// Mock prices.js (functions used for the EURC via-usdc row, not needed here)
...
/** Builds a minimal NormalizedQuote compatible with the engine's ranking. */
...
const AMT = 100_0000000n; // 100 BLND in stroops
...
describe('liveQuote — sourceId / executable', () => {
  ...
  it('exposes sourceId and executable on each row', async () => {
    ...
    // sourceId present and correct
    expect(xbullRow).toBeDefined();
    expect(aquaRow).toBeDefined();

    // executable: xbull = true, aquarius = true (execution venue wired up)
    expect(xbullRow!.executable).toBe(true);
    expect(aquaRow!.executable).toBe(true);
  });

  it('soroswap and horizon are executable', async () => {
    ...
    expect(horizonRow!.executable).toBe(true); // native PathPaymentStrictSend op wired up
  });

  it('re-ranks Aquarius on the simulated net (lower than find-path) → Aquarius loses to xBull', async () => {
    // find-path Aquarius over-quotes (4.02 > 4.00 xBull) → should win WITHOUT the sim
    // with the sim (returns 3.90 < 4.00), xBull should be the winner
    ...
    const fakeSim = vi.fn(async () => 3_9000000n); // simulated net < xBull
    const result = await liveQuote('USDC', AMT, FAKE_CFG as any, { simulateAquariusNet: fakeSim });

    expect(fakeSim).toHaveBeenCalledOnce();
    // xBull should be winner (index 0), aquarius should be non-winner
  ...
  it('re-ranks xBull on the simulated net (0.1% skim) → xBull loses to Aquarius', async () => {
    // xBull over-quotes (4.02) → wins WITHOUT the sim; with the sim (3.90 < 4.00 Aquarius) → Aquarius winner
    ...
    const fakeSimXb = vi.fn(async () => ({ net: 3_9000000n, route: [], transfers: [] })); // simulated xBull net < Aquarius
    const result = await liveQuote('USDC', AMT, FAKE_CFG as any, { simulateXbullNet: fakeSimXb });

    expect(fakeSimXb).toHaveBeenCalledOnce();
    // Aquarius should be winner (index 0), xBull should be non-winner
  ...
  it('composite id (leg1+leg2) → NOT executable (2 non-atomic tx)', async () => {
    ...
    expect(row!.executable).toBe(false); // composite = 2 tx → never 1-click (otherwise we'd execute a direct swap ≠ the one reviewed)
  });
});

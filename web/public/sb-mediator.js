...
var Array2 = class extends NestedXdrType {
   ...
   */
  isValid(value) {
    if (!(value instanceof globalThis.Array) || value.length !== this._length) {
      return false;
    }
    for (const child of value) {
...
async function processTxRequest(client, txRequest) {
  ...
  try {
    ...
    throw errors_default.invalidSwapTx();
  }
...
async function authorizeInvocation(client, tx) {
  ...
  addrAuth.signature(types.ScVal.scvVec([sigScVal]));
}
function signTx(client, tx) {
  return client.authorization.authorize(tx).then((tx2) => {
    if (typeof tx2 === "string") {
      tx2 = TransactionBuilder.fromXDR(tx2, client.network);
    }
    if (!tx2 || tx2.signatures?.length < 1)
      throw new Error(`Transaction was not signed`);
    return tx2;
  }).catch((e) => {
    console.error(e);
    throw errors_default.failedToSignTx();
  });
}
...
function convertToStellarAsset(asset) {
  ...
  const [code, issuer] = asset.includes("-") ? asset.split("-") : asset.split(":");
...
function parseAsset(asset, parameter) {
  ...
  throw errors_default.invalidQuoteParam(parameter, "Invalid asset");
}
function validateCode(code, parameter) {
  if (!/^[a-zA-Z0-9]{1,12}$/.test(code))
    throw errors_default.invalidQuoteParam(parameter, "Invalid asset code: " + (!code ? "missing" : code));
  return code;
}
function validateAccount(account, param) {
  if (!account || !StrKey.isValidEd25519PublicKey(account))
    throw errors_default.invalidQuoteParam(param, "Invalid account address: " + (!account ? "missing" : account));
  return account;
}
...
function validateQuoteRequest(params) {
  ...
  if (res.buyingAsset === res.sellingAsset)
    throw errors_default.invalidQuoteParam("buyingAsset", "Buying asset can't be the same as selling asset");
  if (res.sellingAmount === void 0)
    throw errors_default.invalidQuoteParam("sellingAmount", 'Parameter "sellingAmount" parameter is required');
  Object.assign(res, other);
  Object.freeze(res);
  return res;
}
function parseAmount(amount, parameter) {
  ...
  if (isNaN(parsed) || parsed <= 0)
    throw errors_default.invalidQuoteParam(parameter, "Invalid asset amount: " + amount);
  return amount;
}
function parseSlippageTolerance(src, parameter) {
  const tolerance = parseFloat(src);
  if (isNaN(tolerance))
    throw errors_default.invalidQuoteParam(parameter, "Invalid slippage tolerance, number expected");
  if (tolerance < 0)
    throw errors_default.invalidQuoteParam(parameter, "Slippage tolerance is too small, expected value >= 0");
  if (tolerance > 0.5)
    throw errors_default.invalidQuoteParam(parameter, "Slippage tolerance is too large, expected value < 0.5 (50%)");
  return tolerance;
}
...
var QuoteResult = class {
  ...
  constructor(result) {
     ...
     * @readonly
     */
    __publicField(this, "sellingAsset");
    /**
     * Asset to buy
     * @type {string}
     * @readonly
     */
    __publicField(this, "buyingAsset");
    /**
     * Swap slippage tolerance
     * @type {number}
     * @readonly
     */
    __publicField(this, "slippageTolerance");
    /**
     * Amount of the selling asset
     * @type {string}
     * @readonly
     */
    __publicField(this, "sellingAmount");
    /**
     * Estimated amount of buyingAsset to receive
     * @type {string}
     * @readonly
     */
    __publicField(this, "estimatedBuyingAmount");
    /**
     * Equivalent direct path_payment trade estimate
     * @type {QuoteDirectTradeResult}
     * @readonly
     */
    __publicField(this, "directTrade");
    /**
     * Difference between quoted price and estimated direct trade
     * @type {string}
     * @readonly
     */
    __publicField(this, "profit", "0");
    /**
  ...
  constructor(authorization) {
    ...
    if (typeof authorization === "string") {
      try {
        ...
        throw errors_default.invalidAuthorizationParam();
      }
...
var StellarBrokerClient = class {
  ...
  constructor(params) {
     ...
     * @private
     */
    __publicField(this, "origin", "https://api.stellar.broker");
    /**
     ...
     * @readonly
     */
    __publicField(this, "quoteRequest");
    /**
     * Last quote received from the sever
     * @type {QuoteResult}
     * @readonly
     */
    __publicField(this, "lastQuote");
    /**
     * Current quote accepted for the trading
     * @private
     */
    __publicField(this, "tradeQuote");
    /**
     * API key of the partner
     * @type {string}
     * @readonly
     */
    __publicField(this, "partnerKey");
    /**
  ...
  connect() {
    ...
    this.socket.onclose = () => {
      if (this.status !== "disconnected") {
        ...
        this.notifyError(errors_default.notConnected());
      }
      console.log("Connection closed");
    };
    this.socket.onerror = (e) => console.error(e);
    return new Promise((confirm, reject) => {
      const expirationTimeout = setTimeout(() => reject(this), 2e3);
      this.onSocketOpen = () => {
        this.status = "ready";
        this.heartbeat();
  ...
  processMessage(message) {
    ...
    switch (raw.type) {
      ...
      case "tx":
        ...
        processTxRequest(this, raw).then((xdr) => {
          this.send({
            type: "tx",
            hash: raw.hash,
            xdr
  ...
  quote(params) {
    if (this.status === "trade")
      throw errors_default.tradeInProgress();
  ...
  confirmQuote(account, authorization) {
    if (this.status === "disconnected")
      throw errors_default.notConnected();
    if (this.status === "trade")
      throw errors_default.tradeInProgress();
    if (this.status !== "quote" || !this.lastQuote)
      throw errors_default.quoteNotSet();
    if (/* @__PURE__ */ new Date() - this.lastQuote.ts > 1e4)
      throw errors_default.quoteExpired();
    if (this.lastQuote.status !== "success")
      throw errors_default.quoteError(this.lastQuote.error || "quote not available");
    if (account) {
      this.trader = account;
    }
    if (authorization) {
    ...
    if (!trader || !StrKey.isValidEd25519PublicKey(trader))
      throw errors_default.invalidQuoteParam("account", "Invalid trader account address: " + (!trader ? "missing" : trader));
    if (!this.authorization)
      throw errors_default.invalidQuoteParam("authorization", "Client authorization not provided");
...
var stellarBrokerEvents = /* @__PURE__ */ new Set(["quote", "paused", "progress", "finished", "error"]);
...
var Mediator = class _Mediator {
  ...
  constructor(source, sellingAsset, buyingAsset, sellingAmount, authorization, reserveFeeAmount = 5) {
     ...
     * @private
     */
    __publicField(this, "mediator");
    /**
     * @type {string}
     * @readonly
     */
    __publicField(this, "mediatorAddress");
    /**
     * Prefix of the mediator address record in the localStorage
     * @type {string}
     * @private
     */
    __publicField(this, "storagePrefix", defaultStoragePrefix);
    /**
     * @type {number}
     * @private
     */
    __publicField(this, "reserveFeeAmount");
  ...
  static hasObsoleteMediators(source, storagePrefix = defaultStoragePrefix) {
    return Object.entries(localStorage).some(([key, initiator]) => initiator === source && key.startsWith(storagePrefix));
  }
  /**
   * Retrieve funds from mediator accounts that belong to lost swap sessions
   * @return {Promise}
   */
  async disposeObsoleteMediators() {
  ...
  static async disposeObsoleteMediators(source, authorization, storagePrefix = defaultStoragePrefix) {
    const wrapper = new _Mediator(source, "XLM", "XLM", "0", authorization);
  ...
  async init() {
    ...
    if (sellingAsset.isNative()) {
      const amount = this.sellingAmount + BigInt(feesReserve * 1e7);
      ...
      ops.push(Operation.createAccount({
        source: this.source,
        destination: this.mediatorAddress,
        startingBalance: feesReserve.toString()
        // for tx fees
      }));
    ...
    if (sourceAccount.signers.length > 1) {
      ...
      ops.push(Operation.setOptions({
        source: this.mediatorAddress,
        inflationDest: this.source,
        //store source account to the inflation dest
        homeDomain: "mediator.stellar.broker",
        masterWeight: Math.max(1, thresholds.high_threshold, thresholds.med_threshold),
        //own signer always has the highest weight
  ...
  static createHorizon() {
    return new horizon_exports.Server("https://horizon.stellar.org");
  }
};
function findTrustline(account, asset) {
  return account.balances.find(
    (b) => asset.isNative() ? b.asset_type === "native" : b.asset_code === asset.code && b.asset_issuer === asset.issuer
  );
}
...
function validateStreamedTx(tx, expected) {
  ...
  for (let i = 0; i < ops.length; i++) {
    const op = ops[i];
    if (op.type !== "pathPaymentStrictSend" && op.type !== "pathPaymentStrictReceive" && op.type !== "invokeHostFunction") {
      return {
        ok: false,
    ...
    if (op.type === "invokeHostFunction") {
      ...
      try {
        ...
        const contractId = StrKey.encodeContract(ic.contractAddress().contractId());
        ...
        const fname = String(ic.functionName());
        if (fname !== "swap") {
          return {
            ok: false,
        ...
        let routes;
        try {
          const decoded = scValToNative(rawArgs[1]);
        ...
        if (totalSoldRaw > capRaw) {
          return {
            ok: false,
            reason: `op[${i}] invokeHostFunction: aggregate sold ${totalSoldRaw} stroops > cap ${capRaw} (maxSellAmount=${maxSellAmount} \xD7 TOLERANCE)`
          };
        }
        let txTrader;
        try {
          const decodedTrader = scValToNative(rawArgs[2]);
